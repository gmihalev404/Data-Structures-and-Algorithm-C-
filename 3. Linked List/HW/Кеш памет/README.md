...
### Механика на прегряване
Вашата кеш памет за съжаление не е от най-мощните и от време на време прегрява. Това се случва след изпълнението на **всяка К-та заявка**. Когато прегрее, се изтрива най-отдавна използваният елемент (ако кешът не е вече празен).
...
1.  **$N$** — капацитета на кеш паметта
2.  **$Q$** — броя на заявките
3.  **$K$** — броя на заявки, след които прегрява

На следващите $Q$ реда получавате заявки от типа `put <x> <y>` или `get <x>`.

## Ограничения
* $1 \le N \le 3000$
* $1 \le Q, K \le 2 \cdot 10^5$
* $0 \le x \le 10^4$
* $0 \le y \le 10^5$
